...
fn basic_track_test() {
    ...
    // Test
    let track_dna = TrackDnaData {
        dna_address: DNA_ADDRESS_A.clone(),
        agent_id: ALEX_AGENT_ID.clone(),
    };
    // First track should succeed
    engine.post(Lib3hClientProtocol::TrackDna(track_dna.clone())).unwrap();
    ...
    assert_eq!(res_msg.request_id, "");
    assert_eq!(res_msg.dna_address, DNA_ADDRESS_A.as_slice());
    assert_eq!(res_msg.to_agent_id, ALEX_AGENT_ID.as_slice());
    println!("SuccessResult info: {}", std::str::from_utf8(res_msg.result_info.as_slice()).unwrap());
    // Track same again, should fail
    engine.post(Lib3hClientProtocol::TrackDna(track_dna)).unwrap();
    let (did_work, srv_msg_list) = engine.process().unwrap();
    assert!(did_work);
    assert_eq!(srv_msg_list.len(), 1);
    ...
    assert_eq!(res_msg.dna_address, DNA_ADDRESS_A.as_slice());
    assert_eq!(res_msg.to_agent_id, ALEX_AGENT_ID.as_slice());
    println!("FailureResult info: {}", std::str::from_utf8(res_msg.result_info.as_slice()).unwrap());
    // Done
