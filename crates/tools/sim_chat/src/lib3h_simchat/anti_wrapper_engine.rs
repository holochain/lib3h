...
pub mod tests {
    ...
    struct MockNetworkEngine{
        response_queue: Vec<Lib3hServerProtocol>,
    }
    ...
    impl NetworkEngine for MockNetworkEngine {
        ...
        fn post(&mut self, data: Lib3hClientProtocol) -> Lib3hProtocolResult<()> {            
            println!("post called with {:?}", data);

            match data {
                Lib3hClientProtocol::JoinSpace(space_data) => {
                    self.response_queue.push(Lib3hServerProtocol::SuccessResult(
                        GenericResultData {
    ...
    fn test_round_trip_from_client() {
        let network_engine = MockNetworkEngine::new();
        let mut anti_wrapper = AntiWrapperEngine::new(network_engine);
        ...
        parent_endpoint.request(test_span(""), test_message, Box::new(|_, callback_data| {
            println!("{:?}", callback_data);
            Ok(())
        })).expect("Could not publish a message to the wrapper");
    }
}
