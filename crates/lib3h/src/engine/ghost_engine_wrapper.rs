    ...
    EngineError: ToString,
{
    ...
    pub fn post(&mut self, client_msg: Lib3hClientProtocol) -> Lib3hProtocolResult<()> {
        ...
        if let Ok(client_to_lib3h) = maybe_client_to_lib3h {
            ...
            let ghost_mesage = maybe_ghost_message.ok_or(Lib3hProtocolError::new(
                    ErrorKind::Other(format!("No ghost message for request: {:?}", request_id.as_str()))))?;
                
            ghost_mesage.respond(Ok(lib3h_to_client_response))
