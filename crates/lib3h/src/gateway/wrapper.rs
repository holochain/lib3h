...
pub struct GatewayWrapper<'wrap> {
    gateway: Arc<RwLock<dyn Gateway<'wrap> + 'wrap>>,
...
impl<'wrap> GatewayWrapper<'wrap> {
    ...
    pub fn new<T: Gateway<'wrap> + 'wrap>(concrete: T) -> Self {
    ...
    pub fn as_gateway(&self) -> Arc<RwLock<dyn Gateway<'wrap> + 'wrap>> {
        self.gateway.clone()
    }

    /// immutable ref to the dyn Gateway
    pub fn as_ref(&self) -> RwLockReadGuard<'_, dyn Gateway<'wrap> + 'wrap> {
        self.gateway.read().expect("failed to obtain read lock")
    }

    /// mutable ref to the dyn Gateway
    pub fn as_mut(&self) -> RwLockWriteGuard<'_, dyn Gateway<'wrap> + 'wrap> {
